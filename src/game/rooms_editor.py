import sys
from PyQt6.QtWidgets import QApplication, QWidget, QGridLayout, QLabel, QVBoxLayout, QFileDialog, QComboBox, QPushButton
...
class LevelEditor(QWidget):
    def __init__(self):
        ...
        self.current_symbol = '.'
        self.grid = [['.' for _ in range(self.grid_width)] for _ in range(self.grid_height)]  # Инициализация сетки
        self.dragging = False
        self.initUI()
    ...
    def initUI(self):
        ...
        self.grid_widget.setFixedSize(self.grid_width * self.cell_size, self.grid_height * self.cell_size)
        ...
        self.symbol_selector.addItems(['#', '.', 's', 'p', 'e', 'b', 'B', 't', 'T', 'S', 'c'])
    ...
    def save_level(self):
        filename, _ = QFileDialog.getSaveFileName(self, "Сохранить уровень", "", "Text Files (*.txt)")
        if filename:
